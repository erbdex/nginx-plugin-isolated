...
static int config(const char *key, const char *value) {
    if (strcasecmp(key, "url") == 0)
        ...
        return (config_set(&header, value));
    else
        return (-1);
} /* int config */
...
static int init(void) {
    ...
    if (header != NULL) {
        static char *header_flag = "Host: ";
        char host_header[1024]  = {0};
        
        strcat(host_header, header_flag);
        strcat(host_header, header);
        
        struct curl_slist *curl_list = NULL;
        curl_list = curl_slist_append(curl_list, host_header);
        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, curl_list);
        WARNING("nginx plugin: Header: \"%s\"", host_header);
    ...
    if (url != NULL) {
        WARNING("nginx plugin: URL   : %s", url);
...
static int nginx_read(void) {
    ...
    ptr = nginx_buffer;
    WARNING("nginx caught: %s", nginx_buffer);
    saveptr = NULL;
